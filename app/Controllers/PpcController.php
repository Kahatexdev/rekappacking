...
class PpcController extends BaseController
{
    ...
    public function requestPacking()
    {
        ...
        $daAp=[];
        $daDe=[];
        $dataRequest = $this->dataPDK->getPermintaanPacking();
        foreach ($dataRequest as $dr) {
            $daReq[] = [
                'no_model' => $dr['no_model'],
                'no_order' => $dr['no_order'],
                'buyer' => $dr['buyer'],
            ];
        }
        $dataApprove = $this->dataPDK->getApproved();
        foreach($dataApprove as $da){
            $daAp []= [
                'no_model' => $da['no_model'],
                'no_order' => $da['no_order'],
                'buyer' => $da['buyer'],
            ];
        }
        $dataDecline = $this->dataPDK->getDecline();
        foreach($dataDecline as $dd){
            $daDe []= [
                'no_model' => $dd['no_model'],
                'no_order' => $dd['no_order'],
                'buyer' => $dd['buyer'],
            ];
        }
        $data = [
