...
class PotongCorakController extends BaseController
{
    public function potongcorak()
    {
        ...
        $data = [
            ...
            'Tabel' => 'Data Produksi PotongCorak',
            'Data' => $dataJoined
        ];

        return view('Packing/PotongCorak/potongcorak', $data);
    }

    public function importProduksiPotongCorak()
    {
        ...
        if ($file->isValid() && !$file->hasMoved()) {
                            ...
                            if ($kategori == 'PotongCorak') {
                                ...
                                if ($result && array_key_exists('id_inisial', $result)) {
                                    ...
                                    if ($id_proses && array_key_exists('id_proses', $id_proses)) {
                                        ...
                                        return redirect()->to(base_url('/packing/potongcorak'))->with('error', 'Silahkan input flow proses terlebih dahulu');
                                    }
                                } else {
                                    return redirect()->to(base_url('/packing/potongcorak'))->with('error', 'Silahkan input Master data dan flow proses terlebih dahulu');
                                }
                            } else {
                                return redirect()->to(base_url('/packing/potongcorak'))->with('error', 'Silahkan input DATA PRODUKSI potongcorak (Outflow potongcorak)');
                            }
                        } else {
                            dd('not array found');
                        }
            ...
            return redirect()->to(base_url('/packing/potongcorak'))->with('success', 'Data imported and saved to database successfully');
        } else {
            return redirect()->to(base_url('/packing/potongcorak'))->with('error', 'No data found in the Excel file');
        }
    }
    public function exportPotongCorak($selectedIds)
    {
        ...
        $pdf->Cell(0, 10, 'Export Data Produksi PotongCorak', 0, 1, 'C');
        ...
        $nama = 'export Data Produksi PotongCorak';
