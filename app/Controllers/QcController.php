...
class QcController extends BaseController
{
    public function Qc()
    {
        $dataProduksi = $this->prodModel->getQcData();
        ...
        $data = [
            'Judul' => 'Data Produksi Qc',
            'User' =>  session()->get('username'),
            'Tabel' => 'Data Produksi Qc',
            'Data' => $dataJoined
        ];

        return view('Packing/Qc/qc', $data);
    }

    public function importOutQc()
    {
        ...
        if ($file->isValid() && !$file->hasMoved()) {
                            ...
                            if ($kategori == 'QualityControl') {
                                ...
                                if ($result && array_key_exists('id_inisial', $result)) {
                                    ...
                                    if ($id_proses && array_key_exists('id_proses', $id_proses)) {
                                        ...
                                        return redirect()->to(base_url('/packing/qc'))->with('error', 'Silahkan input flow proses terlebih dahulu');
                                    }
                                } else {
                                    return redirect()->to(base_url('/packing/qc'))->with('error', 'Silahkan input Master data dan flow proses terlebih dahulu');
                                }
                            } else {
                                return redirect()->to(base_url('/packing/qc'))->with('error', 'Silahkan input DATA PRODUKSI qc (Outflow qc)');
                            }
                        } else {
                            dd('not array found');
                        }
            ...
            return redirect()->to(base_url('/packing/qc'))->with('success', 'Data imported and saved to database successfully');
        } else {
            return redirect()->to(base_url('/packing/qc'))->with('error', 'No data found in the Excel file');
        }
    }
    public function exportQc($selectedIds)
    {
        ...
        $pdf->Cell(0, 10, 'Export Data Produksi Qc', 0, 1, 'C');
