...
class QbsController extends BaseController
{
    public function Qbs()
    {
        $dataProduksi = $this->prodModel->getQbsData();
        ...
        $data = [
            'Judul' => 'Data Produksi Qbs',
            'User' =>  session()->get('username'),
            'Tabel' => 'Data Produksi Qbs',
            'Data' => $dataJoined
        ];

        return view('Packing/Qbs/qbs', $data);
    }

    public function importOutQbs()
    {
        ...
        if ($file->isValid() && !$file->hasMoved()) {
                ...
                if (!empty($data)) {
                    ...
                    if ($data[0] == null) {
                        ...
                        if ($result && array_key_exists('kategori', $result)) {
                            ...
                            if ($kategori == 'QualityBeforeSetting') {
                                ...
                                if ($result && array_key_exists('id_inisial', $result)) {
                                    ...
                                    if ($id_proses && array_key_exists('id_proses', $id_proses)) {
                                        ...
                                        return redirect()->to(base_url('/packing/qbs'))->with('error', 'Silahkan input flow proses terlebih dahulu');
                                    }
                                } else {
                                    return redirect()->to(base_url('/packing/qbs'))->with('error', 'Silahkan input Master data dan flow proses terlebih dahulu');
                                }
                            } else {
                                return redirect()->to(base_url('/packing/qbs'))->with('error', 'Silahkan input DATA PRODUKSI qbs (Outflow qbs)');
                            }
                        } else {
                            dd('not array found');
                        }
            ...
            return redirect()->to(base_url('/packing/qbs'))->with('success', 'Data imported and saved to database successfully');
        } else {
            return redirect()->to(base_url('/packing/qbs'))->with('error', 'No data found in the Excel file');
        }
    }
    public function exportQbs($selectedIds)
    {
        ...
        $pdf->Cell(0, 10, 'Export Data Produksi Qbs', 0, 1, 'C');
