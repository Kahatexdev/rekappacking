...
class LipatController extends BaseController
{
    ...
    public function importProduksiLipat()
    {
        ...
        if ($file->isValid() && !$file->hasMoved()) {
                            ...
                            if ($kategori == 'Lipat') {
                                ...
                                if ($result && array_key_exists('id_inisial', $result)) {
                                    ...
                                    if ($id_proses && array_key_exists('id_proses', $id_proses)) {
                                        ...
                                        return redirect()->to(base_url('/packing/lipat'))->with('error', 'Silahkan input flow proses terlebih dahulu');
                                    }
                                } else {
                                    return redirect()->to(base_url('/packing/lipat'))->with('error', 'Silahkan input Master data dan flow proses terlebih dahulu');
                                }
                            } else {
                                return redirect()->to(base_url('/packing/lipat'))->with('error', 'Silahkan input DATA PRODUKSI lipat (Outflow lipat)');
                            }
                        } else {
                            dd('not array found');
                        }
            ...
            return redirect()->to(base_url('/packing/lipat'))->with('success', 'Data imported and saved to database successfully');
        } else {
            return redirect()->to(base_url('/packing/lipat'))->with('error', 'No data found in the Excel file');
        }
    }
